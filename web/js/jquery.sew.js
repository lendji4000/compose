...
(function ($, window, undefined) {
	...
	Plugin.prototype.onItemClick = function (element, e) {
        ...
        this.$element.trigger("sew.element.click", [element]);
	};
	...
	$.fn[pluginName] = function (options) {
		return this.each(function (index) {
			if(!$.data(this, 'plugin_' + pluginName)) {
                instances[index] = new Plugin(this, options);

				$.data(this, 'plugin_' + pluginName, instances);
			}
		});
	};

    $.fn.sew.refreshValues = function(values){
        for( key in instances ){
            instances[key].options.values = values;
        }
    }
}(jQuery, window));
